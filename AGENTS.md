...
本项目用于《席德·梅尔的文明VI》游戏模组开发与维护，主要使用 Lua、SQL 与 `.modinfo` 文件实现建造者种植资源、地块校验、本地化与加载配置。

## 项目目标

维护并迭代 Civilization VI 模组 `BP_ResourcePlanter`，确保资源种植、菜单显示、本地化文本和视觉表现稳定可用。

本工作区的核心目标是围绕 `BP_ResourcePlanter` 做最小、可验证的模组修改，优先修复 SQL 数据层、Lua gameplay 脚本、`.modinfo` 配置和文本资源中的真实问题。

## 核心工作流

当用户提出 Civilization VI 模组相关需求时，按以下流程执行：
...
修改 SQL 数据层 -> 调整 Lua Gameplay 脚本 -> 校验 modinfo 与本地化文本 -> 在游戏内加载模组并验证建造菜单、资源生成与视觉表现
